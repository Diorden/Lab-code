...
void EXTI9_5_IRQHandler (void) {  // Inturrupt Handler
    EXTI->PR = EXTI_PR_PR6;
  	measure();
}
...
void measure(void) {
	...
	while ((ADC->CSR & 0x02) != 0x02) {			// Waits until ADC end-of-conversion bit is set high
		...
		resistance = calculate_resistance(measured_voltage);  // Resistance reading should be selected by a mode selector in final design
		snprintf(buffer, 8, "%f", measured_voltage); // Replace measured_voltage with resistance to test resistance readings
		PB_LCD_Clear();
